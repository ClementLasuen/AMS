...
for o in range(compteur):
    ...
    while(True):
        ...
        number_killed += len(replicas_to_kill)
...
plt.ylabel('Probabilité en moyenne')
plt.title('Convergence en moyenne de la probabilité')
plt.plot(X,[sum(PROBA[:i])/(i+1) for i in range(len(PROBA))])
plt.show()